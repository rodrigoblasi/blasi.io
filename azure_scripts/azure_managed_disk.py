...
"""This script collects metrics from azure Managed Disks."""
...
def parse_arguments():
    ...
    parser.add_argument("--metric-name", dest="metric_name",
                        help="Azure availables metrics: [ 'Composite_Disk_Read_Bytes/sec',\
                        'Composite_Disk_Read_Operations/sec','Composite_Disk_Write_Bytes/sec',\
                        'Composite_Disk_Write_Operations/sec']",
                        required=True, type=str)
...
def main():
    ...
    try:
        ...
        metric_name = azure_base.remove_underscore(args.metric_name)
        ...
        resource_id = azure_tags.get_resource_id_by_tag(args.sys_id,\
            credentials,subscription_id,'microsoft.compute/disks')
        ...
        azure_errors.throws('managed-disks')
