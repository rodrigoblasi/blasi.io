...
"""This script collects metrics from azure Load Balancer."""
...
def parse_arguments():
    ...
    parser.add_argument("--metric-name", dest="metric_name",
                        help="Azure availables metrics: [ 'VipAvailability','DipAvailability'\
                        'SYNCount','ByteCount','PacketCount']",
                        required=True, type=str)
...
def main():
    ...
    try:
        ...
        resource_id = azure_tags.get_resource_id_by_tag(args.sys_id,\
            credentials,subscription_id,'Microsoft.Network/loadBalancers')
        ...
        azure_errors.throws('load-balancer')
