...
"""This script collects metrics from azure SQL Database."""
...
def parse_arguments():
    ...
    parser.add_argument("--metric-name", dest="metric_name",
                        help="Azure availables metrics: ['cpu_percent','storage',\
                            'storage_percent','tempdb_data_size',\
                            'tempdb_log_size']",
                        required=True, type=str)
...
def main():
    ...
    try:
        ...
        resource_id = azure_tags.get_resource_id_by_tag(args.sys_id,\
            credentials,subscription_id,'Microsoft.Sql/servers/databases')
        ...
        if any(metric in args.metric_name for metric in ("tempdb_data_size", "tempdb_log_size")):
            output_metric = azure_base.convert_to_bytes(str(output_metric) + " KB")

        print(output_metric)

    except:
        azure_errors.throws('sql-database')
