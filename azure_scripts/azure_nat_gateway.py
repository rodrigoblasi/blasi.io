...
"""This script collects metrics from azure NAT Gateway."""
...
def parse_arguments():
    ...
    parser.add_argument("--metric-name", dest="metric_name",
                        help="Azure availables metrics: [ 'ByteCount','PacketCount'\
                        'PacketDropCount','SNATConnectionCount','TotalConnectionCount',\
...
def main():
    ...
    try:
        ...
        resource_id = azure_tags.get_resource_id_by_tag(args.sys_id,\
            credentials,subscription_id,'Microsoft.Network/natGateways')
        ...
        azure_errors.throws('nat-gateway')
